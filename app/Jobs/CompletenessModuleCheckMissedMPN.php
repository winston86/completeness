...
/**
 * Class CompletenessModuleCheckMissedMPN
 *
 * @author r.ratsun@treolabs.com
 */
class CompletenessModuleCheckMissedMPN extends Base
{

    static $error_type = 'MissedMPN';
    static $severity  = 'Low';
    ...
    public function run()
    {
        ...
        foreach ($products as $key => $product) {
            ...
            $item['name']       = $this->getInjection('language')->translate('Missed MPN', 'names', 'CompletenessModuleError');
            $item['product_id'] = $product->get('id');
            $item['error']      = $this->error_type;
            $item['message'] = sprintf($this->getInjection('language')->translate('Missed MPN in %s', 'messages', 'CompletenessModuleError'), $product->get('id'));
            $items[] = $item;
