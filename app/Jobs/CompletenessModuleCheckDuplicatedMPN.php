...
/**
 * Class CompletenessModuleCheckDuplicatedMPN
 *
 * @author r.ratsun@treolabs.com
 */
class CompletenessModuleCheckDuplicatedMPN extends Base
{

    static $error_type = 'DuplicatedMPN';
    ...
    public function run()
    {
        ...
        foreach ($duplicates as $key => $d) {
           ...
           $products = $this
            ...
            ->where([
                'Product.deleted' => 0,
                'Product.mpn' => $d->get('mpn')
            ]);

            foreach ($products as $key => $product) {

                $item['severity']   = $this->severity;
                $item['name']       = $this->getInjection('language')->translate('Duplicated MPN', 'names', 'CompletenessModuleError');
                $item['product_id'] = $product->get('id');
                $item['error']      = $this->error_type;
                $item['message'] = sprintf($this->getInjection('language')->translate('Duplicated MPN in %s', 'messages', 'CompletenessModuleError'), $product->get('id'));
