...
/**
 * Class CompletenessModuleCheckCategorySet
 *
 * @author r.ratsun@treolabs.com
 */
class CompletenessModuleCheckCategorySet extends Base
{
    static $error_type = 'CategoryNotSet';
    static $severity  = 'Low';
    ...
    public function run()
    {
        ...
        foreach ($products as $key => $product) {
           ...
           $item['name']       = $this->getInjection('language')->translate('Missed Category', 'names', 'CompletenessModuleError');
           $item['product_id'] = $product->get('id');
           $item['error']      = $this->error_type;
           $item['message'] = sprintf($this->getInjection('language')->translate('Missed Category in %s', 'messages', 'CompletenessModuleError'), $product->get('id'));
