...
/**
 * Class CompletenessModuleCheckMissedSKU
 *
 * @author r.ratsun@treolabs.com
 */
class CompletenessModuleCheckMissedSKU extends Base
{
    

    static $error_type = 'MissedSKU';
    static $severity  = 'Low';
    ...
    public function run()
    {
        ...
        foreach ($products as $key => $product) {
            ...
            $item['name']       = $this->getInjection('language')->translate('Missed SKU', 'names', 'CompletenessModuleError');
            $item['product_id'] = $product->get('id');
            $item['error']      = $this->error_type;
            $item['message'] = sprintf($this->getInjection('language')->translate('Missed SKU in %s', 'messages', 'CompletenessModuleError'), $product->get('id'));
            $items[] = $item;
